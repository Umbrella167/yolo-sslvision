...
def generate_images(path, start_frame, end_frame, step, quality, size=None, output_dir="resource/output_images",is_continue = True):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    max_frame_id = 0
    output_dir = os.path.join("resource/output_images")
    if not is_continue:
            os.path.exists(output_dir) and os.system("rm -rf output_images")
    else:
        ...
        for file in existing_files:
            if file.endswith('.jpg'):
                ...
                max_frame_id = max(max_frame_id, frame_id) + 1
    ...
    saved_frame_count = max_frame_id  # Track how many frames have been saved
