...
class RandomRules:
    ...
    def load_image(self, image):
        self.sam2.load_image(image)
    def magic_mask_sam2(self, image, point):
        masks, scores, logits = self.sam2.add_point([point], [1])
        return masks[0]
    def magic_mask_sam2_points(self, image, points,labels):
        masks, scores, logits = self.sam2.add_point(points, labels)
        return masks[0]
    def magic_mask(self, image, point, color_buffer, dist):
        ...
        # 获取目标颜色
        target_color = hsv_image[int(point[1]), int(point[0])]
        ...
        # # 计算每个连通区域的质心到point的距离
        # distances_to_point = [
        #     utils.calculate_distance(point, (int(centroid[0]), int(centroid[1])))
        #     for centroid in centroids[1:]
        # ]

        # # 找到距离point最近的连通区域
        # closest_label = 1 + np.argmin(distances_to_point)

        # # 创建最终掩码，仅保留最近的连通区域
        # closest_mask = np.zeros_like(final_mask)
        # closest_mask[labels == closest_label] = 255
    ...
    def get_mark_data(self, image, rectangles,robot_quantity = 9999):
        ...
        for count, (x, y, w, h) in enumerate(rectangles):
            ...
            robot_data = randomrules.get_robot_circle_sam2(image_robot_yuv)
            if len(robot_data) >= 3:
